...
/**
 * Complete Inventory Service - TRANSACTION SAFETY PATCHED
 * 
 * CRITICAL FIXES APPLIED:
 * - Explicit rollback on all error paths in invoice processing
 * - BOM deduction returns structured results (no uncaught exceptions)
 * - Consistent structured returns from processSimpleInvoice()
 * 
 * @version 3.1.0 - Transaction Safety Patch
 * @date 2025-11-30
 */
class CompleteInventoryService
{
     ...
     *
     * Rewritten to return structured array results and to ensure explicit rollback
     * on all error paths. Stored procedure calls are still used for stock movement,
     * but failures are handled cleanly and return a helpful error_code when possible.
     */
    public function processSimpleInvoice(int $salesOrderId, int $userId): array
    {
        try {
            ...
            // Get sales order details (single-line order assumed)
            $stmt = $this->db->prepare("
                SELECT 
                    so.sales_order_id,
                    so.customer_name,
            ...
            if (!$order) {
                // Explicit rollback and structured error
                if ($this->db->inTransaction()) {
                    $this->db->rollBack();
                }
                return [
            ...
            // Check stock availability (for non-BOM items)
            ...
            if ($order['is_bom_item']) {
                ...
                // Regular item - process stock out via stored procedure
                $stmt = $this->db->prepare("
                ...
                if (!$result || !$result['success']) {
                    ...
                    return [
                        'success' => false,
                        'message' => $result['message'] ?? 'Unknown error during stock movement',
                        'error_code' => 'STOCK_MOVEMENT_FAILED'
                    ];
                }
            }
            ...
            // Get new stock level for response
     ...
     *
     * Returns structured array instead of throwing so caller can rollback
     * and return appropriate messages.
     */
        ...
        try {
            ...
            foreach ($components as $comp) {
                ...
                if (!$result || !$result['success']) {
                    return [
                        'success' => false,
                        'message' => "BOM component error: " . ($result['message'] ?? 'Unknown error'),
