...
class SupplierController
{
    ...
    /*Get all suppliers (ADMIN ONLY) ✅ FIXED: Returns array properly */
    public function getAll()
    {
        ...
        try {
            ...
            // ✅ If no suppliers exist, return empty array (not null)
            if (!$suppliers) {
                $suppliers = [];
            }

            // ✅ Log the response structure for debugging
            error_log("📦 Suppliers fetched: " . count($suppliers) . " records");
            error_log("📦 First supplier (if any): " . json_encode($suppliers[0] ?? 'none'));
