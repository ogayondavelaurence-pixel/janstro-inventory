...
// PARSE REQUEST PATH
// ============================================================================
// ✅ For PDF/file downloads, don't use output buffering
// ✅ CRITICAL: Disable output buffering for file downloads
$requestUri = $_SERVER['REQUEST_URI'] ?? '';
$isPdfOrFileDownload = (
    strpos($requestUri, '/pdf') !== false ||
    strpos($requestUri, '/download') !== false ||
    strpos($requestUri, 'invoices') !== false && strpos($requestUri, 'pdf') !== false
);

if ($isPdfOrFileDownload) {
    // Kill all output buffers BEFORE routing
    while (ob_get_level()) {
        ob_end_clean();
    }
    error_log("🔧 Output buffering disabled for: {$requestUri}");
} else {
    // Start output buffering for normal requests
    ob_start();
}
...
try {
    ...
    /* ================================================================
       EMAIL SETTINGS ROUTES (SUPERADMIN ONLY)
    ================================================================ */
    ...
    if ($resource === 'purchase-orders') {
        ...
        if ($method === 'POST' && is_numeric($action) && $sub === 'deliver') {
            $order->markDelivered((int)$action);
            exit;
        }
    ...
    if ($resource === 'stock-requirements') {
        ...
        if ($method === 'GET') {
            $stmt = $db->query("
                SELECT sr.*, so.customer_name, so.order_date, i.item_name
            ...
            Response::success(['requirements' => $requirements], 'Stock requirements retrieved');
            exit;
        }
    ...
    if ($resource === 'purchase-requisitions') {
        ...
        if ($method === 'GET' && $action === '') {
            $stmt = $db->query("
                SELECT pr.*, i.item_name, u.name as requested_by_name
            ...
            Response::success(['requisitions' => $prs], 'Purchase requisitions retrieved');
            exit;
        }
    ...
    if ($resource === 'bom') {
        AuthMiddleware::authenticate();
        require_once __DIR__ . '/api/bom-api.php';
        exit;
    }
