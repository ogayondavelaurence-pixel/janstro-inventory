...
if ($method === 'POST' && is_numeric($action) && $id === 'payment') {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT invoice_number, total_amount, payment_status, pdf_path FROM invoices WHERE invoice_id = ?");
        $stmt->execute([$invoiceId]);
        $invoice = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $stmt = $db->prepare("SELECT COALESCE(SUM(amount), 0) as total_paid FROM invoice_payments WHERE invoice_id = ?");
        $stmt->execute([$invoiceId]);
        ...
        // Commit BEFORE PDF regeneration
        $db->commit();
        ...
        try {
            // Fetch FRESH invoice data with new status
            $stmt = $db->prepare("
                SELECT i.*, so.delivery_address, so.contact_number as customer_phone
                FROM invoices i
                LEFT JOIN sales_orders so ON i.sales_order_id = so.sales_order_id
            ...
            $stmt = $db->prepare("
                SELECT ii.*, i.item_name, i.sku, i.unit
                FROM invoice_items ii
                LEFT JOIN items i ON ii.item_id = i.item_id
                WHERE ii.invoice_id = ?
            ");
            ...
            $pdfService = new PdfService();
            $newPdfPath = $pdfService->generateInvoicePDF($invoiceData, $lineItems);
            ...
            $pdfRegenerated = true;
            error_log("✅ PDF regenerated with status: {$newStatus}");
        } catch (Exception $e) {
        ...
        $stmt->execute([
            $user->user_id,
            "Recorded payment of PHP " . number_format($amount, 2) . " for Invoice #{$invoice['invoice_number']} | Method: {$paymentMethod} | Status: {$newStatus}",
            $_SERVER['REMOTE_ADDR'] ?? 'unknown'
        ]);
        ...
        Response::success([
            ...
            'pdf_path' => $newPdfPath ?? $invoice['pdf_path'],
            'cache_buster' => time()
